...
import com.ingenio.gestion_logistica.interfaces.DetalleEnvioInterface;
import com.ingenio.gestion_logistica.model.Clientes;
import com.ingenio.gestion_logistica.model.DetalleEnvio;
import com.ingenio.gestion_logistica.model.Envio;
import com.ingenio.gestion_logistica.model.Operaciones;
...
public class DetalleEnvioDao implements DetalleEnvioInterface{
	...
	public String agregarDetalleEnvio(int idcliente,DetalleEnvio datos) throws Exception {
		...
		double vunitario = p.getPROD_VALOR();
		double vcantidad = datos.getENVD_CANTIDAD();
		Integer ttrasporte = dtenvio.getENV_TRAS_ID();

		double valneto = vunitario * vcantidad;
		
		Operaciones  po  = realisarOperaciones(vunitario,vcantidad,valneto,ttrasporte);
	...
	public Operaciones realisarOperaciones(double vunitario,double vcantidad,double valneto,Integer ttrasporte) {
		...
		if (vcantidad >= 10 && ttrasporte == 1) {
			descuento = valneto * 5 / 100;
			subtotald = valneto - descuento;
		}

		if (vcantidad >= 10 && ttrasporte == 2) {
